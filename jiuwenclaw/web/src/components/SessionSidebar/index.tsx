...
type SidebarTabKey = 'settings' | 'blank-1' | 'blank-2';
...
export function SessionSidebar({
  ...
  const [activeTab, setActiveTab] = useState<SidebarTabKey>('settings');

  const renderPlaceholder = () => (
    <div className="session-sidebar-placeholder">
      <div className="session-sidebar-placeholder__title">{t('sessionSidebar.blankTitle')}</div>
      <p className="session-sidebar-placeholder__body">{t('sessionSidebar.blankBody')}</p>
    </div>
  );
  ...
  return (
    <aside className="nav session-sidebar flex flex-col">
      <div className="session-sidebar-tabs">
        ...
        <button
          type="button"
          className={`session-sidebar-tab ${activeTab === 'blank-1' ? 'is-active' : ''}`}
          onClick={() => setActiveTab('blank-1')}
        >
          {t('sessionSidebar.tabs.blankOne')}
        </button>
        <button
          type="button"
          className={`session-sidebar-tab ${activeTab === 'blank-2' ? 'is-active' : ''}`}
          onClick={() => setActiveTab('blank-2')}
        >
          {t('sessionSidebar.tabs.blankTwo')}
        </button>
      </div>
