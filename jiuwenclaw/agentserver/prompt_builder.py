...
- **用户画像：** `memory/USER.md`（稳定的身份属性与偏好信息）
- **知识沉淀：** `memory/MEMORY.md`（经筛选提炼的长期背景知识，非原始流水账））

#### 历史检索机制

**响应任何消息前，建议执行：**
1. **身份确认** — 读取 `memory/USER.md` 确认服务对象
2. **上下文获取** — 读取 `memory/YYYY-MM-DD.md`（当日 + 前一日）
3. **长期记忆加载** — **仅限主会话：** 读取 `memory/MEMORY.md`
...
- **User Profile:** `memory/USER.md` (Stable identity attributes and preference information.)
- **Knowledge Repository:** `memory/MEMORY.md` (Filtered and refined long-term background knowledge, not raw logs.)

#### History Retrieval Mechanism

**Before responding to any message, it is recommended to execute:**
1. Read `memory/USER.md` — Confirm the user being served
2. Read `memory/YYYY-MM-DD.md` (today + previous day) to get context
3. **Main session only:** Read `memory/MEMORY.md`
...
- 更新 memory/USER.md 或 memory/MEMORY.md 时，必须先读取现有内容再执行修改
...
1. 更新前必须先读取现有内容
2. 合并新信息，避免全量覆盖
3. memory/MEMORY.md 条目仅记录精炼事实，不含日期/时间戳
4. **memory/USER.md 字段去重：** 已存在字段通过 `edit_memory` 更新，不存在字段通过 `write_memory` 追加
...
### Core Operation Guidelines
 ...
 - When updating memory/USER.md or memory/MEMORY.md, existing content must be read first before making modifications.	 
...
1. **User Profile Information (`user_profile`)**: Stable user attributes such as identity, preferences, habits, work style, likes/dislikes.
   - **Storage**: Write to `memory/USER.md`.
...
3. **Semantic Memory (`semantic_memory`)**: Long-term background knowledge, technical details, and tool-related local configs (SSH, camera, etc.).
   - **Storage**: Write to `memory/MEMORY.md`.
...
**Before responding to any message, it is recommended to execute:**
""" + _en_exp_retrieve_item + """2. Read `memory/USER.md` — Confirm the user being served
3. Read `memory/YYYY-MM-DD.md` (today + previous day) to get context
4. **Main session only:** Read `memory/MEMORY.md`
...
3. memory/MEMORY.md entries should only record refined facts, without dates/timestamps
4. **memory/USER.md Field Deduplication:** Existing fields should be updated via `edit_memory`, non-existing fields should be appended via `write_memory`
"""
        sections.append(memory_mgmt_prompt)
